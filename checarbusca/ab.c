...
//temos que reutilizar as coisa do estritamentebin e fazer uma função pra decidir se uma arvore é de busca 
...
/*funçoes juntas*/
int checa_busca(NO *no){
    if(no->esq == NULL || no->dir == NULL)
        return 1; //alcancei uma folha

    if(item_get_chave(no->esq->item) >= item_get_chave(no->dir->item)) //caso a folha da esquerda seja maior (ou igual, importante!) retorna 0 pq nao é de busca
        return 0;
    else //caso contrario eu desço mais na arvre
        return (checa_busca(no->esq) && checa_busca(no->dir));
}
...
int ab_checar_busca(AB *t){
    ...
    return !checa_busca(t->raiz); //denovo, por algum motivo o retorno é ao contrario igual o outro exercicio (funcoes de cima)
}
/*--------------*/